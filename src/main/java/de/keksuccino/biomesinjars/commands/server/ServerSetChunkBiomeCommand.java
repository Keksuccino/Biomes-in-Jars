        ...
        stack.getServer().execute(() -> {
            try {
                ...
                if (WorldUtils.setChunkBiomeAtBlockPos(stack.getLevel(), stack.getPlayerOrException().blockPosition(), stack.getLevel().registryAccess().registryOrThrow(Registry.BIOME_REGISTRY).getResourceKey(biome).get())) {
                    stack.sendSuccess(new TextComponent("§aChunk biome changed to: " + stack.getLevel().registryAccess().registryOrThrow(Registry.BIOME_REGISTRY).getResourceKey(biome).get().location().toString()), false);
                } else {
                    stack.sendFailure(new TextComponent("§cUnable to change biome!"));
                }
