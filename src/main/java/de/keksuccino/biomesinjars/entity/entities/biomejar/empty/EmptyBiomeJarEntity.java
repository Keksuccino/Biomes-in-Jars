...
public class EmptyBiomeJarEntity extends Mob implements IAnimatable {
    ...
    public void tick() {
        ...
        if (!biomeHolder.is(de.keksuccino.biomesinjars.biome.Biomes.DEAD_LAND)) {
...
//                    this.vibrateAnimationState.startIfStopped(this.tickCount);
    ...
    protected void setDeadLandBiomeAt(BlockPos blockPos) {
        WorldUtils.setChunkBiomeAtBlockPos(this.level, blockPos, Biomes.DEAD_LAND);
    ...
    protected InteractionResult mobInteract(Player player, InteractionHand interactionHand) {
        if (!this.level.isClientSide) {
            ...
            if (slot != -1) {
                player.getInventory().setItem(slot, new ItemStack(Items.EMPTY_BIOME_JAR_ITEM));
            } else {
                ItemUtils.dropItemStack(this.level, new ItemStack(Items.EMPTY_BIOME_JAR_ITEM), this.blockPosition());
            }
        }
    ...
    public static EmptyBiomeJarEntity spawnAt(ServerLevel serverLevel, BlockPos blockPos, float xRot, boolean b) {
        EmptyBiomeJarEntity entity = Entities.EMPTY_BIOME_JAR_ENTITY.create(serverLevel);
        if (entity == null) {
            return null;
        }
...
//    private <E extends IAnimatable> PlayState animationPredicate(AnimationEvent<E> e) {
//        if (this.level.isClientSide) {
//            Holder<Biome> biomeHolder = this.level.getBiome(this.blockPosition());
//            if (!biomeHolder.is(de.keksuccino.biomesinjars.biome.Biomes.DEAD_LAND)) {
